          ...
          >
            ...
            >
              <span style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                ...
                <span style={{ fontFamily: FONT_MONO, fontSize: 11.5, color: 'rgba(232,234,242,.55)', whiteSpace: 'nowrap' }}>{range}</span>
                <span style={{ fontSize: 13.5, color: 'rgba(232,234,242,.82)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                ...
                <span style={{ fontFamily: FONT_MONO, fontSize: 12, color: 'rgba(232,234,242,.4)', transform: isOpen ? 'rotate(90deg)' : 'none', transition: 'transform .15s' }}>
                  ›
                </span>
              </span>
            </button>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  {x.signals.map((s, j) => (
                    <div key={j} style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                      <span style={{ color: s.concerning ? tone : 'rgba(232,234,242,.4)', fontSize: 13 }}>•</span>
                      <span style={{ fontSize: 13.5, color: 'rgba(232,234,242,.8)', lineHeight: 1.45 }}>{s.text}</span>
                    </div>
                  ))}
                </div>
                <div style={{ fontSize: 13.5, color: 'rgba(232,234,242,.7)', lineHeight: 1.5, textWrap: 'pretty' }}>
                  <span style={{ fontFamily: FONT_MONO, fontSize: 10, letterSpacing: '.12em', color: 'rgba(232,234,242,.4)', textTransform: 'uppercase' }}>Why</span>
                  <div style={{ marginTop: 2 }}>{x.meaning}</div>
                </div>
                <div style={{ fontSize: 13.5, color: 'rgba(232,234,242,.7)', lineHeight: 1.5, textWrap: 'pretty' }}>
                  <span style={{ fontFamily: FONT_MONO, fontSize: 10, letterSpacing: '.12em', color: 'rgba(232,234,242,.4)', textTransform: 'uppercase' }}>What you could do</span>
