  ...
  return (
    <div style={{ ...card(16), borderColor: rgba(r.color, 0.35), padding: '20px 24px', marginTop: 24, ...rise(0.1, 0.5) }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 14.5, color: 'rgba(234,234,234,.7)' }}>{r.headline}</span>
        {r.asOf && <span style={mono(10, 'rgba(234,234,234,.35)')}>as of {formatShortDate(r.asOf)}</span>}
      </div>
      {(r.drivers.length > 0 || r.load) && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 18px', marginTop: 10 }}>
          {r.drivers.map((d) => (
            <Link key={d.key + d.text} href={`/signal?m=${d.key}`} style={{ ...mono(11, d.concerning ? r.color : 'rgba(234,234,234,.55)'), textDecoration: 'none' }}>
              • {d.text} →
            </Link>
          ))}
          {r.load && <span style={mono(11, 'rgba(234,234,234,.5)')}>• {r.load}</span>}
        </div>
      )}
      <div style={{ ...mono(9.5, 'rgba(234,234,234,.32)'), letterSpacing: '.08em', marginTop: 10 }}>
