...
export default function DashboardPage() {
  ...
  return (
    <div style={{ paddingTop: 64 }}>
      ...
      <p style={{ fontSize: 14.5, color: 'rgba(234,234,234,.5)', marginTop: 10, maxWidth: 560, lineHeight: 1.5 }}>
      ...
      >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
                  ...
                  <span
                    style={{
                      ...mono(10, active ? t.meta.color : 'rgba(234,234,234,.45)'),
                      letterSpacing: '.14em',
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 12 }}>
                <span style={{ ...mono(24, '#eaeaea'), letterSpacing: '-.01em' }}>{t.cur}</span>
                <span style={mono(11.5, 'rgba(234,234,234,.45)')}>{t.meta.unit}</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginTop: 12 }}>
                ...
                <span style={mono(10.5, 'rgba(234,234,234,.4)')}>{t.zTxt}</span>
              </div>
            </button>
          )
        })}
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            <span style={{ ...mono(10, 'rgba(234,234,234,.45)'), letterSpacing: '.14em', textTransform: 'uppercase' }}>
              Instruments
            </span>
            <span
              style={{
          ...
          {sources.length === 0 ? (
            <div style={{ ...mono(11.5, 'rgba(234,234,234,.42)'), marginTop: 15, lineHeight: 1.5 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginTop: 15 }}>
              {sources.slice(0, 4).map((s) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: 12.5, color: 'rgba(234,234,234,.7)' }}>{s.name}</span>
                    <span
                      style={{
                        ...mono(9, g.color),
                        letterSpacing: '.1em',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginTop: 16 }}>
            <span
              style={{
                ...mono(10, distrusted > 0 ? COLOR.coral : 'rgba(234,234,234,.4)'),
                letterSpacing: '.1em',
                textTransform: 'uppercase',
              }}
            >
            ...
            <span style={mono(10.5, 'rgba(234,234,234,.4)')}>graded per export</span>
          </div>
        </Link>
      </div>
      ...
      {episodes.length > 0 && (
        <div style={{ ...card(16), padding: '22px 26px 20px', marginTop: 14, ...rise(0.44, 0.55) }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            ...
            <span style={{ ...mono(10.5, 'rgba(234,234,234,.32)'), letterSpacing: '.12em' }}>
              {`${episodes.length} IN 90 DAYS`}
            </span>
          </div>
          <p style={{ fontSize: 13.5, color: 'rgba(234,234,234,.5)', margin: '8px 0 14px', maxWidth: 620, lineHeight: 1.5 }}>
      ...
      {heatmap.hasData && (
        <div style={{ ...card(16), padding: '22px 26px 18px', marginTop: 14, ...rise(0.46, 0.55) }}>
          ...
          <p style={{ fontSize: 13, color: 'rgba(234,234,234,.5)', margin: '8px 0 4px', maxWidth: 620, lineHeight: 1.5 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px 18px', marginTop: 14, alignItems: 'center' }}>
            {[
              ...
              <span key={l.t} style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={mono(10, 'rgba(234,234,234,.5)')}>{l.t}</span>
              </span>
            ))}
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 14, height: 12, borderRadius: 2, background: 'repeating-linear-gradient(45deg, transparent, transparent 2px, rgba(234,234,234,.18) 2px, rgba(234,234,234,.18) 3px)', border: '1px solid rgba(234,234,234,.06)' }} />
              <span style={mono(10, 'rgba(234,234,234,.5)')}>no data</span>
            </span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 14, height: 8, borderRadius: 2, background: COLOR.coral }} />
              <span style={mono(10, 'rgba(234,234,234,.5)')}>combined alert</span>
            </span>
          </div>
        </div>
      )}
      ...
      <div style={{ ...card(16), padding: '22px 26px', marginTop: 14, ...rise(0.48, 0.55) }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <span style={{ ...mono(10.5, 'rgba(234,234,234,.32)'), letterSpacing: '.12em' }}>90 DAYS</span>
            <Link
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 12, flexWrap: 'wrap' }}>
          ...
          {breakdown.bandRange && band?.status !== 'no_data' && band?.status !== 'data_gap' && (
            <span style={{ fontSize: 13, color: 'rgba(234,234,234,.45)' }}>{breakdown.bandRange}</span>
          )}
        </div>

        <div style={{ marginTop: 10 }}>
          <MetricChart result={result} metricKey={selected} height={210} />
        </div>
      ...
      <div style={{ ...card(16), padding: '20px 24px 14px', marginTop: 14, ...rise(0.54, 0.55) }}>
        <div style={{ ...mono(11, 'rgba(234,234,234,.55)'), letterSpacing: '.16em', textTransform: 'uppercase' }}>
          ...
          {metricDecisions.map((e, i) => {
            ...
            return (
              ...
              >
                ...
                <div style={{ fontSize: 13, color: 'rgba(234,234,234,.78)', lineHeight: 1.45, textWrap: 'pretty' }}>
                  {e.title}
                </div>
                {e.lines.length > 0 && (
                  <div style={{ ...mono(10.5, 'rgba(234,234,234,.45)'), lineHeight: 1.5 }}>
