...
const label: React.CSSProperties = { fontFamily: FONT_MONO, fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(237,234,226,.55)' }
...
function SignalDetail() {
  ...
  return (
    <div style={{ paddingTop: 48 }}>
      {/* Back */}
      <Link href="/dashboard" style={{ ...mono(11, 'rgba(237,234,226,.5)'), letterSpacing: '.1em', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginTop: 20, ...rise(0.06) }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ ...mono(10.5, 'rgba(237,234,226,.42)'), letterSpacing: '.18em' }}>METRIC · LAST 90 DAYS</div>
      ...
      {(() => {
        ...
        const col = good ? COLOR.teal : excluded > 0 || cov < 40 ? COLOR.amber : 'rgba(237,234,226,.5)'
        return (
          <div style={{ ...mono(10.5, col), letterSpacing: '.08em', marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <span style={{ color: 'rgba(237,234,226,.45)' }}>
              {excluded > 0
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginTop: 14 }}>
          {breakdown.stats.map((s, i) => (
            <div key={s.label} style={{ ...card(14), padding: '16px 18px', ...rise(0.2 + i * 0.03) }}>
              <div style={{ ...mono(10, 'rgba(237,234,226,.42)'), letterSpacing: '.14em', textTransform: 'uppercase' }}>{s.label}</div>
              <div style={{ ...mono(19, '#edeae2'), marginTop: 9 }}>{s.value}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {findings.length > 0 && (
        <>
          <div style={{ ...mono(10.5, 'rgba(237,234,226,.4)'), letterSpacing: '.18em', marginTop: 40 }}>FINDINGS ON THIS SIGNAL</div>
            ...
            {findings.map((f) => (
              <div key={f.decision.date + f.title} style={{ ...card(18), borderColor: rgba(f.color, 0.3), padding: '22px 26px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                  ...
                  <span style={mono(11, 'rgba(237,234,226,.38)')}>{f.date}</span>
                </div>
                <h2 style={{ fontSize: 18, fontWeight: 600, margin: '14px 0 0', letterSpacing: '-.01em' }}>{f.title}</h2>
                {f.body && <p style={{ fontSize: 14.5, lineHeight: 1.55, color: 'rgba(237,234,226,.66)', margin: '9px 0 0', maxWidth: 760, textWrap: 'pretty' }}>{f.body}</p>}
              </div>
            ))}
          </div>
        </>
        ...
        {instruments.length > 0 && (
          <div style={{ ...card(16), padding: '20px 24px' }}>
            ...
            <div style={{ marginTop: 10 }}>
              {instruments.map((r, i) => (
                <div key={r.source} style={{ padding: '12px 0', borderTop: i > 0 ? '1px solid rgba(237,234,226,.06)' : undefined, display: 'flex', flexDirection: 'column', gap: 7 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                    <span style={{ fontSize: 13.5, color: 'rgba(237,234,226,.85)' }}>{r.source}</span>
                  ...
                  {r.note && <div style={{ fontSize: 12.5, color: 'rgba(237,234,226,.48)', lineHeight: 1.45 }}>{r.note}</div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ ...card(16), padding: '20px 24px 14px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: 9, flex: 1 }}>
            ...
            {decisions.map((e, i) => {
              ...
              return (
                <div key={i} style={{ padding: '11px 0', borderTop: i > 0 ? '1px solid rgba(237,234,226,.06)' : undefined, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <div style={{ fontSize: 13, color: 'rgba(237,234,226,.78)', lineHeight: 1.45, textWrap: 'pretty' }}>{e.title}</div>
                </div>
              )
            })}
          </div>
          <button
            ...
            style={{ ...mono(11, 'rgba(237,234,226,.5)'), letterSpacing: '.1em', textTransform: 'uppercase', background: 'none', border: 'none', borderTop: '1px solid rgba(237,234,226,.06)', padding: '14px 0 6px', marginTop: 8, textAlign: 'left', cursor: 'pointer', width: '100%' }}
          >
