  ...
  return (
    <div style={{ paddingTop: 76 }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0 }}>
                ...
                <span style={{ fontFamily: FONT_DISPLAY, fontSize: 16, fontWeight: 600, letterSpacing: '.005em', color: '#eaeaea' }}>
                  {cat.title}
                </span>
              </span>
            ...
            <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 2 }}>
              {cat.metrics.map((m) => (
                ...
                >
                  <span style={{ fontSize: 12.5, color: 'rgba(234,234,234,.6)' }}>{m.name}</span>
                  <span style={{ fontFamily: FONT_MONO, fontSize: 12.5, color: '#eaeaea' }}>{m.cur}</span>
                </Link>
              ))}
            </div>
          </div>
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                ...
                <span style={{ fontFamily: FONT_MONO, fontSize: 11, letterSpacing: '.1em', color: 'rgba(234,234,234,.38)' }}>
                  {f.date}
                </span>
              </div>
              ...
              {isOpen && (
                <div style={{ marginTop: 18, borderTop: '1px solid rgba(234,234,234,.08)', paddingTop: 18 }}>
                  {f.meta && (
                    <BandChart
      ...
      >
        ...
        <span style={{ fontSize: 14.5, lineHeight: 1.6, color: 'rgba(234,234,234,.55)' }}>{verdict.steady}</span>
      </div>
      <div style={{ fontFamily: FONT_MONO, fontSize: 11.5, color: 'rgba(234,234,234,.32)', marginTop: 14, ...rise(0.46, 0.55) }}>
        {verdict.nextNote}
      </div>
    </div>
  )
